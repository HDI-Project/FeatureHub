...
def test_get_source():
    ...
    code_f     = featurehub.util.get_source(f)
    code_g     = featurehub.util.get_source(g)
    code_h     = featurehub.util.get_source(h)
    code_zeros = featurehub.util.get_source(zeros)
...
def test_get_function():
    ...
    for fn in [f, g, h, w, x, y, z]:
        code = featurehub.util.get_source(fn)
        function = featurehub.util.get_function(code)
...
def test_get_function2():
    ...
    for fn in [f, g, h, w, x, y, z]:
        code = featurehub.util.get_source(fn)
        function = featurehub.util.get_function2(code)
        ...
        try:
            for i in range(n):
                code = featurehub.util.get_source(function)
                function = featurehub.util.get_function2(code)
        except Exception:
            pass
...
def test_compute_dataset_hash():
    ...
    dataset_hash = featurehub.util.compute_dataset_hash(dataset)

    # should get the same hash if we recompute with no changes
    assert dataset_hash == featurehub.util.compute_dataset_hash(dataset)
    assert dataset_hash == featurehub.util.compute_dataset_hash(dataset)
    assert dataset_hash == featurehub.util.compute_dataset_hash(dataset)
...
def test_myhash():
    ...
    assert featurehub.util.myhash(a) == featurehub.util.myhash(b)

def test_is_positive_env():
    for y in ["yes", "Yes", "y", "Y", "true", "True", True, 1, "1"]:
        assert featurehub.util.is_positive_env(y) == True

    for n in ["", "no", "No", "blargh", "zam", "zot", False, 0, "0"]:
        assert featurehub.util.is_positive_env(n) == False
